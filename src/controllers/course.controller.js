...
export const getAllCourses = async (req, res) => {
    ...
    if(req.query.populate){
        if(req.query.populate.toLowerCase() === 'teacher'){
            include.push(db.Teacher);
        }
    else if (req.query.populate.toLowerCase() === 'student') {
        ...
        const courses = await db.Course.findAll(
            {
                ...
                limit: limit, offset: (page - 1) * limit,order: [['createdAt', sort]],include: include.length ? include : undefined
            }
        );
        res.json({
            meta: {
